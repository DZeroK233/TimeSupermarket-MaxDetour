...
class SupermarketSolverApp:
    def __init__(self, root):
        self.root = root
        self.root.title("超市最优布局求解器 - 多尺寸连体货架精准打包版")
    ...
    def setup_ui(self):
        ...
        tk.Label(control_frame, text="(放不下货架的死角将变为0收益障碍物)", font=("Arial", 9), fg="gray").pack(anchor=tk.W)
    ...
    def evaluate_layout(self, path_set, blocked_set, config):
        """
        【全新二维空间贪心打包算法 - 严格侧边屏蔽】
        精准区分 1x2 货架的长宽，彻底拒绝在默认情况下利用侧边结算销量！
        并搜集每个路径格子的触货数，形成路径热力图。
        """
        ground_set = set()
        for r in range(self.rows):
            for c in range(self.cols):
